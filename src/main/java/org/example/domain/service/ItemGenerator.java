...
public class ItemGenerator {
    ...
    public static List<Item> generateForLevel(int level) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (roll < 8) {
                // 8% шанс — сокровища
                item = new Item("treasure", "Gold", 0, 0, 0, 0,
                        50 + rand.nextInt(100) + level * 40);
            }
            else if (roll < 28) {
                // 20% — еда
                item = new Item("food", "Food", 8 + level + rand.nextInt(6), 0, 0, 0, 0);
            }
            else if (roll < 50) {
                // 22% - эликсиры
                boolean isStrength = rand.nextBoolean();
                String name = isStrength
                        ? "Strength Potion (+" + (4 + level) + ")"
                        : "gility Potion (+" + (4 + level) + ")";
                ...
                String name = rand.nextBoolean()
                        ? "Blade (damage +" + (4 + level + rand.nextInt(3)) + ")"
                        : "Sword (damage +" + (3 + level + rand.nextInt(4)) + ")";
